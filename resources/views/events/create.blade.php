...
<body>
    ...
    <div class="container-fluid mt-5 px-5">
        ...
        <form method="POST" action="{{ route('events.store', ['id' => $restaurant->id]) }}">
            @csrf
            <div class="row mb-3">
                <div class="col-md-4"> <label for="date" class="form-label">Data</label>
                ...
                <div class="col-md-6">
                    ...
                    <select name="event_type_id" id="event_type_id" class="form-select" required>
                        ...
                        @foreach ($eventTypes as $type)
                            <option value="{{ $type->id }}" @selected(old('event_type_id') == $type->id)> {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mt-3">
                ...
                <button type="submit"name="action" value="event" class="btn btn-success"> Utwórz wydarzenie </button>
            </div>
        </form>
    </div>
