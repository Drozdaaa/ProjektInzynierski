        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content text-start">
                <div class="modal-header">
                    ...
                    @if (Auth::check() && isset($event) && Gate::allows('manage-event', $event))
                        <a href="{{ route('menus.user-create', ['restaurant' => $event->restaurant_id, 'event' => $event->id]) }}"
                            class="btn btn-success btn-sm ms-3">
                            <i class="bi bi-plus-lg"></i> Skomponuj dodatkowe menu
                        </a>
                    @endif
                                ...
                                <div class="btn-group" role="group">
                                    @php
                                        ...
                                        if (isset($event) && Auth::check()) {
                                            $canManage = Gate::allows('manage-event', $event);
                                    ...
                                    @if ($canManage)
                                        ...
                                        @if (!$isAttached)
                                            <form
                                                ...
                                                @csrf
                                                <button type="submit" class="btn btn-success btn-sm text-nowrap ms-1">
                                ...
                                <ul class="list-group mb-3">
                                    @foreach ($dishes as $dish)
                                        <li class="list-group-item text-start">
                                            ...
                                            <div class="mb-2 text-muted small">{{ $dish->description }}</div>
                                            <div class="mb-1" style="font-size: 0.85rem;">
                                                ...
                                                @else
                                                    @foreach ($dish->diets as $diet)
                                                        <span
                                                            class="text-success fw-semibold me-1">{{ $diet->name }}</span>
                                                    @endforeach
                                                @endif
                                            </div>
                                            <div style="font-size: 0.85rem;">
                                                ...
                                                @else
                                                    @foreach ($dish->allergies as $allergy)
                                                        <span
                                                            class="text-danger fw-semibold me-1">{{ $allergy->name }}</span>
                                                    @endforeach
                                                @endif
                                            </div>
                                        </li>
